...
- **Multiplicative spot-price adjustment now uses main's audited implementation** — Beta's standalone `spot_multiplier` / `export_spot_multiplier` port (9.9.0b1–b5) is replaced by the version ported to production main ([#227](https://github.com/johanzander/bess-manager/pull/227)), which folds in additional fixes found during code review: the wizard's provider-switch handler and re-scan-on-mount path no longer clobber a saved custom multiplier with the generic default, and `setup_complete()`'s live-update path now applies both fields immediately instead of requiring a restart. Behavior for Belgian/ENTSO-e users is unchanged; this is no longer beta-only.
...
- **Battery/Home/Price settings startup and PATCH paths unified** — Both paths previously reached `BatterySystemManager` through separate hand-maintained `*_STORE_TO_API` translation registries and a `camelCase`-to-`snake_case` conversion that was load-bearing for startup but a no-op for PATCH. A field present in the store but missing from a registry (e.g. `charging_power_rate`, `efficiency_charge`, `efficiency_discharge`) was silently dropped at startup while still working live via PATCH, reverting to class defaults on every restart. `snake_case` is now the single canonical format for both paths, and the registries are replaced by dataclass-derived field sets kept in sync by a structural test. A stale pre-migration key (e.g. `consumption` coexisting with `default_hourly`) is now filtered out on the PATCH path the same way the startup path already handled it, instead of crashing with `AttributeError`. ([#197](https://github.com/johanzander/bess-manager/pull/197), [#216](https://github.com/johanzander/bess-manager/pull/216), [#219](https://github.com/johanzander/bess-manager/pull/219), [#224](https://github.com/johanzander/bess-manager/pull/224))
- **Solcast detection failed with non-English entity names** — Solcast sensors were matched via `entity_id` substrings, which fails when Home Assistant translates entity names to the user's language. This was already fixed on beta but never reached main because the only call site never passed the entity registry through; now wired via `SOLCAST_SUFFIX_MAP` and included in the debug bundle exporter's registry filters. ([#223](https://github.com/johanzander/bess-manager/pull/223))
