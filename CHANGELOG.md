...
- **Beta add-on installs separately** — beta builds now use name "BESS Manager (Beta)" and slug `bess_manager_beta`, so the beta can be installed and toggled alongside the stable add-on. (beta packaging only — not part of the stable release)
