...
Everything else accumulated in `Unreleased` above (the DP backward-induction rework, the export-miscrediting/threshold fixes, ENTSO-e/Belpex, the settings-path unification, Solcast, anti-cycling, GRID_CHARGING throttling, LOAD_SUPPORT) already reached beta testers in v9.9.0b5–v9.9.0b9 under the old release process, just via separately-authored commits rather than the same commit landing on both repos. This release covers only what's genuinely new since v9.9.0b9.
