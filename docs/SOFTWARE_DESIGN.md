...
Neither is user-overridable — polarity is a hardware fact, not configuration. Both splits activate only when the two keys resolve to the same entity_id. For signed battery platforms, `derive_platform_sensor_aliases()` supplies the discharge key from the configured charge key both when registry discovery runs and whenever `flatten_sensors()` builds the active mapping. Manual/custom-integration configurations therefore expose one battery-power field while satisfying both logical consumers. `BESSController.refresh_power_polarities()` re-syncs both after any settings change that can switch platform.
